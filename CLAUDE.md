...
2. **Plan the implementation.** Before writing code, enter plan mode and draft an implementation plan that maps the issue's acceptance criteria to concrete changes (files, layers, test approach). Get the user's explicit approval on the plan before exiting plan mode and starting implementation.
3. **Implement on the branch.** Run typechecking and the affected test files regularly, and the full suite (`pnpm typecheck && pnpm test`) once at the end. For `apps/web` changes, launch the app with `/run` and walk the golden path and edge cases in the browser before considering the feature done. Commit with Conventional Commit messages, proposing each message to the user before committing.
4. **Update the issue.** When the feature is finished, update the GitHub issue (`gh issue edit`): verify each acceptance criterion against the delivered result before checking it off, and only check criteria that are fully satisfied at that point. This is a first pass — the fresh-session review in the gate step re-verifies it independently. Do not close the issue by hand — the merge does it (see below).
5. **Open a pull request.** Push the branch and create the PR with `gh pr create`. Give it a Conventional Commit title (it becomes the squash commit on `master`); let it pre-fill the body from `.github/PULL_REQUEST_TEMPLATE.md`, then adapt each section to the actual change and put `Closes #N` in the `## Closes` section so the merge closes the issue automatically (sub-issue progress rolls up to its epic).
6. **Gate the PR.** CI (typecheck + tests + issue-link check) must be green. From a fresh session — not the one that implemented the change — run `/code-review medium --comment` for correctness and simplification, and `/review` for fidelity to the issue's acceptance criteria; both post as inline PR comments. Always pass the `medium` effort level explicitly — never invoke `/code-review` with a free-text argument (e.g. a scope description), since an unrecognized target falls back to `high` effort and can spawn a costly wide scan instead of reviewing the diff. Reserve `high` or `ultra` for changes that are unusually large or risky. For changes touching auth, sessions, or user access/roles, also run `/security-review`. Every CONFIRMED finding must be fixed (or explicitly justified as not applicable) before merge — a posted comment thread alone is not sufficient.
7. **Merge.** After the user approves, merge with `gh pr merge --squash --delete-branch`. Update the GitHub milestone descriptions or the project README only if the delivery changes a milestone, the delivery order, or the visible scope.
