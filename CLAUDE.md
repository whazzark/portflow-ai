...
4. **Open a pull request.** Push the branch and create the PR with `gh pr create`. Give it a Conventional Commit title (it becomes the squash commit on `master`); let it pre-fill the body from `.github/PULL_REQUEST_TEMPLATE.md`, then adapt each section to the actual change and put `Closes #N` in the `## Closes` section so the merge closes the issue automatically (sub-issue progress rolls up to its epic).
