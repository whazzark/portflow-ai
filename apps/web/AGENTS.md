...
- A detail panel is a `Sheet` at `data-[side=right]:sm:max-w-lg` — `SheetContent` sets its own width under that same variant, and a plain `sm:max-w-lg` loses to it on specificity instead of replacing it — built from `ResourceDetailHeader` / `ResourceDetailBody` / `SheetFooter`. `SheetTitle` names the dialog, so scope it in tests with `getByRole('dialog')` rather than a labelled region.
