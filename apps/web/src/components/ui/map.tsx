...
import { classnames as cn } from '@/libraries/shadcn/helpers'
...
type MapProps = {
  ...
  loading?: boolean
  /** Called when MapLibre cannot initialize or load the active map style. */
...
function MarkerTooltip({ children, className, ...popupOptions }: MarkerTooltipProps) {
  ...
  useEffect(() => {
    ...
    const handleFocusOut = () => tooltip.remove()

    marker.getElement()?.addEventListener('mouseenter', handleMouseEnter)
    marker.getElement()?.addEventListener('mouseleave', handleMouseLeave)
    marker.getElement()?.addEventListener('focusin', handleFocusIn)
    marker.getElement()?.addEventListener('focusout', handleFocusOut)

    return () => {
      marker.getElement()?.removeEventListener('mouseenter', handleMouseEnter)
      marker.getElement()?.removeEventListener('mouseleave', handleMouseLeave)
      marker.getElement()?.removeEventListener('focusin', handleFocusIn)
      marker.getElement()?.removeEventListener('focusout', handleFocusOut)
      tooltip.remove()
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [map])
