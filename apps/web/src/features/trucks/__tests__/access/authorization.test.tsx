...
test.each([ACTIVE_OBSERVER, ACTIVE_OPERATIONS_LEAD])(
  ...
  async (user) => {
    ...
    const { router } = renderTrucks('/transport-resources?truckStatus=archived')
...
test.each([ACTIVE_OBSERVER, ACTIVE_OPERATIONS_LEAD])(
  ...
  async (user) => {
    ...
    const { router } = renderTrucks(
      ...
      '/transport-resources?truckStatus=archived&truckId=00000000-0000-4000-8000-000000000103',
    )
...
test('uses the complete endpoint and exposes archived consultation to administrators', async () => {
  ...
  expect(truckTab(/Archived \(1\)/)).toBeInTheDocument()
