...
Every environment other than automated tests relies on the cookie for session state: there is no session persistence layer to run, back up, or scale, but a session's data is bounded by cookie size and lives entirely on the client between requests. Because the server holds nothing it could delete, ending a session from the server side is done by refusing it when it is presented; ADR 0015 records the one case that needs it, a session opened before its user's latest reactivation. Session restoration (`GET /auth/me`) and logout (`POST /auth/logout`) reuse the same guard and cookie store: both require the `web` guard through `auth_middleware`, and neither carries business logic beyond reading or clearing the authenticated user.
