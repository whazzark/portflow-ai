...
export default class extends BaseSchema {
  ...
  async up() {
    ...
    this.defer(async (db) => {
      await db.rawQuery('CREATE UNIQUE INDEX docks_name_unique ON docks (LOWER(name))')
      await db.rawQuery('CREATE INDEX docks_status_index ON docks (status)')
    })
  }
