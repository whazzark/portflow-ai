...
3. **Verify the behavior.** Before opening the PR, run `/verify` to exercise the change end-to-end rather than trusting automated tests alone.
...
5. **Open a pull request.** Push the branch and create the PR with `gh pr create`. Give it a Conventional Commit title (it becomes the squash commit on `master`) and put `Closes #N` in the body so the merge closes the issue automatically; sub-issue progress rolls up to its epic.
