...
- Maintain independent Dock and Weighing Area selections, each scoped to that kind's current status and search result. In the combined view, render a resource-labelled toolbar for each non-empty selection and dispatch each action only to its resource-specific grouped endpoint; never combine kinds in one request. Changing a kind's search or status clears only that kind's selection and prior grouped feedback, while hiding a kind clears its selection. Grouped requests submit each selected row's current version as `expectedVersion` and show an inline announced outcome that lists every changed and unchanged resource rather than relying on a summary toast. Clear changed IDs; retain only still-visible blocked IDs, and disable stale retry until an explicit refetch supplies current versions.
...
- **Web feature**: render through the real router/providers with MSW. Before MapCN implementation, drive the client-only canvas, SSR-safe synchronized list, distinct type icons, map-unavailable fallback, and accessible map/list representation from a focused RED seam that does not require WebGL. Cover URL-restored independent per-type state, name-only substring search, status filtering, observer read-only behavior, GPS create/edit forms, marker/list-backed detail sheets, individual lifecycle, simultaneous per-kind selections and labelled toolbars in the combined view, resource-specific grouped requests, independent scope clearing, mixed grouped results, stale reload behavior, cache invalidation, loading/retry/empty states, accessible names, and keyboard flow. After the relevant UI exists, verify responsive containment at viewport widths `375`, `768`, `1024`, and `1440` px: the document must have no page-level horizontal overflow (`document.documentElement.scrollWidth <= window.innerWidth`), and the map/list, filters, detail sheet or dialog, and selection toolbars must remain visible and keyboard reachable without horizontal page scrolling.
