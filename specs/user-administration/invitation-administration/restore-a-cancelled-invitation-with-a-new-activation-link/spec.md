...
1. **Given** a cancelled user's access record is open, **When** an organization admin looks for access actions, **Then** the restoration is offered on that record, labelled "Restore invitation".
...
- **FR-015**: The workbench MUST offer the restoration from a cancelled user's access record and from that user's row menu in the collection, labelled "Restore invitation" in both places, with the same confirmation, the same refusals, and the same outcome.
- **FR-016**: The workbench MUST require an explicit confirmation that names the user concerned, offers an optional comment, and states that their invitation will be pending again, that a new activation link valid for 7 days will be shown once, and that any link they were given before stays unusable. Its confirm button MUST read "Restore invitation" and its dismiss button "Cancel". Dismissing it MUST issue nothing, record nothing, and discard the typed comment; a refusal MUST keep the typed comment.
...
- The action is labelled "Restore invitation": "invitation" names what is restored, as in "Cancel invitation" and "Renew activation link". The confirmation keeps the product-wide "Cancel" dismiss button, since no other button in it starts with "Cancel".
