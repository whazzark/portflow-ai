...
| State | Shows |
|---|---|
| Open | Title "Add shift". Description "Plan a new shift for {vessel}: its period, its responsible, and the resources it will use." On an active discharge, the description drops "and the resources it will use" and adds "Its resources are chosen once it is added". Fields: Responsible (combobox, eligible responsibles), Duration hint, Planned start, Planned end, then on a planned discharge only: Weighing areas, Warehouse doors, Trucks, with the same rows, locked notes, and empty-state links (`Go to product lots`, `Go to truck pool`) as the shift correction. Footer: `Add shift` submit. |
